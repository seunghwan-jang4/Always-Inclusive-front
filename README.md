...
![image](https://github.com/user-attachments/assets/b62111e4-0d54-41f5-b891-a523ca94fbf8)
...
2. **추천 시스템**
   - 사용자가 선호하는 카테고리를 요구하면 관련 콘텐츠 추천.
   - 유사 콘텐츠 추천 알고리즘 적용.

3. **사용자 인터페이스**
   - 직관적인 UI로 카테고리 선택 및 추천 결과 확인 가능.
   - 프로필 이미지, 챗 봇 이미지 변경 가능.
   - 이름 변경 가능.
   - 나이대, 성별 변경 가능.
...
- **Database**: MariaDB
- **Frontend**: Uizard, Figma, Vanilla JS
...
![image](https://github.com/user-attachments/assets/613223c8-783c-4ded-bdb3-39694207164e)
...
전체적으로 항상 발등에 불 떨어진 느낌으로 진행한 것 같아 아쉬운 마음이 듭니다.

첫 번째로, 팀원들 간의 협업에 차질이 생기지 않고 원활하게 진행되기 위해서는 명확한 데드라인 설정과 의사소통이  매우 중요함을 깨달았습니다.

두 번째는, 프로젝트 선정 이유, 기술적 의사결정 등 각 선택에 대한 당위성과 그 당위성을 뒷받침해 줄 명확한 근거가 필수적이라고 느꼈습니다.
...
| 김건태   | 크롤링     | 데이터 크롤링 및 문서화                            |
| 박수호B  | 백엔드  | LangChain 활용 데이터 처리 및 RAG 시스템 구현 |
| 이명혜   | 크롤링     | 데이터 크롤링 및 및 문서화                    |
